...
export const StudentRegistration: React.FC<StudentRegistrationProps> = ({ onBack, onSuccess }) => {
  ...
  const generateLoginId = (phoneNumber: string) => {
    ...
    return `S${lastFourDigits}`;
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!formData.name || !formData.registerNumber || !formData.year || !formData.branch || !formData.phoneNumber || !formData.email || !formData.password) {
      toast({
        title: "Error",
          ...
          <div className="space-y-2">
            ...
            {formData.phoneNumber.length >= 4 && (
              <p className="text-sm text-muted-foreground">
                Your login ID will be: S{formData.phoneNumber.slice(-4)}
              </p>
            )}
          </div>
